...
function Products() {
    // we are not using useState bcz whenever component is mounted again state(filters) gets reset , so we will keep our filters inside url
    ...
    const { data: products } = useQuery({
        // all parameters that are using in url should by passed here
        queryKey: ['products', limit, skip, q, category],
        queryFn: async () => {
            let url = `https://dummyjson.com/product/search?limit=${limit}&skip=${skip}&q=${q}`
            if (category) {
        ...
        //holds back previous data until new comes
